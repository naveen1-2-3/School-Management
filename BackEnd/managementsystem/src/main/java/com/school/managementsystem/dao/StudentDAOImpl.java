...
public class StudentDAOImpl implements StudentDAO {
    ...
    public Student insertStudentPaymentSimpl(StudentFee stuFee, MultipartFile file) {
        ...
        // Handling Insertion + Image If Uploaded
        if (!file.getOriginalFilename().equals("null")) {
            ...
            // Handling Updation
            int id = stuFee.getStudent().getId();

            Student stu = getStudentById(id);

            stuFee.getStudent().setImageData(stu.getImageData());
            stuFee.getStudent().setImageName(stu.getImageName());
        }
    ...
    public List<Student> getStudentsByYearStandard(int year, String standard) {
        TypedQuery<Student> stu = entityManager.createQuery(
                "FROM Student s JOIN StudentFee sf ON s.id = sf.student.id WHERE sf.year =:year AND sf.standard =:standard",
                Student.class);
    ...
    public StudentFee getStudentWithFeeById(int id) {
        ...
        TypedQuery<StudentFee> stuFee = entityManager.createQuery("FROM StudentFee sf WHERE sf.student.id = :id",
                StudentFee.class);
    ...
    public String deleteStudentById(int id) {
        TypedQuery<StudentFee> sfQuery = entityManager.createQuery("FROM StudentFee sf WHERE sf.student.id = :id",
                StudentFee.class);
